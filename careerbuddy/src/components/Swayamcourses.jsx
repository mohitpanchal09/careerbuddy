...
export default function Swayamcourses() {
  ...
  const currentItems = (filtered.length ? filtered : data.courses).slice(itemOffset, endOffset)
  const pageCount = Math.ceil((filtered.length ? filtered : data.courses).length / itemsPerPage)

  const handlePageClick = event => {
    const newOffset =
      (event.selected * itemsPerPage) % (filtered.length ? filtered : data.courses).length
          ...
          {currentItems.map(c => (
            <Container>
              <InnerContainer>
                ...
                <Text>
                  Professor: <p style={{padding: '0px 20px', color: 'gray'}}>{c.professor}</p>
                </Text>
              </InnerContainer>
